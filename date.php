...
$_SESSION['arrival1']=$arrival;
$_SESSION['departure1']=$departure;
$_SESSION['adults1']=$adults;
$_SESSION['children1']=$children;
...
<div class="form_panel"><br>
     ...
     <form name="form1" method="GET" action="book.php" >
       ...
       <label class="form_heading" style="float:left; margin-top:0px; margin-left:20px;">ADULTS</label>
        <select name="adults"  style="float:left; margin-top:20px; width:100px; height:29px; margin-left:-50px;">
           <!-- <option selected="selected" value="<?php echo $adults?>"><?php echo $adults?></option>-->
            <?php
            if($adults==1)
            {
              echo '<option selected="selected" value='.$adults.'>'.$adults.'</option><option value="2">2</option>
            <option value="3">3</option>';  
            }
            if($adults==2)
            {
                echo '<option value="1">1</option><option selected="selected" value='.$adults.'>'.$adults.'</option><option value="3">3</option>';
            }
            if($adults==3)
            {
                echo '<option value="1">1</option>
            <option value="2">2</option><option selected="selected" value='.$adults.'>'.$adults.'</option>';
            }
            ?>
       ...
       <select name="rooms" style="float:left; margin-top:20px; width:100px; height:29px; margin-left:-45px;">
            ...
            if($rooms==1)
            {
              echo '<option selected="selected" value='.$rooms.'>'.$rooms.'</option><option value="2">2</option>
            <option value="3">3</option><option value="4">4</option>
            <option value="5">5</option>';  
            }
            if($rooms==2)
            {
                echo '<option value="1">1</option><option selected="selected" value='.$rooms.'>'.$rooms.'</option><option value="3">3</option><option value="4">4</option>
            <option value="5">5</option>';
            }
            if($rooms==3)
            {
                echo '<option value="1">1</option>
            <option value="2">2</option><option selected="selected" value='.$rooms.'>'.$rooms.'</option><option value="4">4</option>
            <option value="5">5</option>';
            }
            if($rooms==4)
            {
                echo '<option value="1">1</option>
            <option value="2">2</option>
            <option value="3">3</option><option selected="selected" value='.$rooms.'>'.$rooms.'</option><option value="5">5</option>';
            }
            if($rooms==5)
            {
                echo '<option value="1">1</option>
            <option value="2">2</option>
            <option value="3">3</option>
            <option value="4">4</option><option selected="selected" value='.$rooms.'>'.$rooms.'</option>';
            }
            ?>
        ...
        <label class="form_heading" style="float:left; margin-top:0px; margin-left:20px;">CHILDREN</label>
         <select name="children"  style="float:left; margin-top:20px; width:100px; height:29px; margin-left:-65px;">
            ...
            if($children==0)
            {
              echo '<option selected="selected" value='.$children.'>'.$children.'</option><option value="1">1</option>
            <option value="2">2</option>';  
            }
            if($children==1)
            {
                echo '<option value="0">0</option><option selected="selected" value='.$children.'>'.$children.'</option><option value="2">2</option>';
            }
            if($children==2)
            {
                echo '<option value="0">0</option>
            <option value="1">1</option><option selected="selected" value='.$children.'>'.$children.'</option>';
            }
            ?>    
...
<div id="section11"><br>
    ...
    <?php
        ...
        $result4 = mysqli_query($con,"select room_no,type from room_details where adults='$adults' and child='$children' and Avail='not available'");
    	while($row4=mysqli_fetch_array($result4))
        {
        	if($row4['arrival']<$arrival && $row4['departure']>$departure){
            if($row4['type']=='suite')
                $count_suite++;
            if($row4['type']=='deluxe')
                $count_deluxe++;
    ...
    if($count_deluxe>0)
    {
        ...
        echo '<div id="bar22" >&nbsp&nbsp&nbsp&nbspEXECUTIVE DELUXE  with King Bed<br>&nbsp&nbsp&nbsp&nbspFrom <c style="color:#cd592c;">INR&nbsp&nbsp'.$row1['price'].' </c> / night 
      <input type="button" name="book_now1" value="BOOK NOW " class="bk_nw1" onClick="book_del()"/>
    </div>
    <div id="feature" >
    ...
    //| <c1 onMouseOver="this.style.color='.$blue.'" onMouseOut="this.style.color='.$brown.'"style="color:brown; font-size:15px;" onClick="details()">Room Details</c1>
     elseif($count_suite>0)
    {
        
        echo '<div id="bar22" >
        &nbsp&nbsp&nbsp&nbspEXECUTIVE SUITE  with Twin Bed<br>&nbsp&nbsp&nbsp&nbspFrom <c id="suite_id" style="color:#cd592c;">INR&nbsp&nbsp'.$row2['price'].' </c> / night 
      <input type="submit" name="book_now1" value="BOOK NOW " class="bk_nw1" onClick="book_suite()"/>
    </div>
    <div id="feature" >
    <b>Includes buffet breakfast and internet!<b>
    </div>
     ...
     elseif($count_normal>0)
    {
        ...
        echo '<div id="bar22" >&nbsp&nbsp&nbsp&nbspEXECUTIVE Rooms  with King Bed<br>&nbsp&nbsp&nbsp&nbspFrom <c id="normal_id" style="color:#cd592c;">INR&nbsp&nbsp'.$row3['price'].'</c> / night 
      <input type="submit" name="book_now1" value="BOOK NOW " class="bk_nw1" onClick="book_nor()"/>
    </div>
    <div id="feature" >
    <b>Includes buffet breakfast and internet!<b>
    </div>
...
<script type="text/javascript">
    ...
    function book_del(){
       ...
       location.href='book_user.php?name=del';//or window.location.href='';
    }
    function book_suite(){
        
       location.href='book_user.php?name=suite';//or window.location.href='';
    }
    function book_nor(){
        
       location.href='book_user.php?name=nor';//or window.location.href='';
    }
